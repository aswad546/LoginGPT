...
MONGO_PORT = 27019
DB_NAME = "sso-monitor"
# Collection name as a global variable that can be easily modified
TARGET_COLLECTION = "banks_8500_result"
...
def main():
    ...
    # Find total count of ALL documents in the collection, not just those with api_status=0
    total_count = collection.count_documents({})
    logger.info(f"Found {total_count} documents in collection {TARGET_COLLECTION}")
    ...
    while processed_count < total_count:
        ...
        # Removed the api_status: 0 filter to process all documents
        query = {}
        if last_id:
